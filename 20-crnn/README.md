...
图中的 B 是一次处理几张图；H、W 是输入高宽；H′、W′ 是卷积和池化后的高宽；C 是特征通道数。变成序列以后，用 T 表示帧数、F 表示每帧有几个特征，再用 h 表示每个方向的 LSTM 状态宽度。最后 K 是字符类别数加一个 blank。

以本章一张 8×20 图为例，池化后有 4 行、10 列、4 通道。**横着有 10 列，就读 10 步；每一列的 4 行×4 通道合成 16 个数。** 所以序列形状是 `[1,10,16]`，不是“有 10 个字符”。整串 `001` 只有三个字符，如何把 10 步对应到它们，交给 CTC。

后面每节只负责把这条链往前推进一段，最终再检查损失能否一路传回最前面的卷积核。
...
每张图有 8×20=160 个数，按一行从左到右、再读下一行的顺序放进 `Float32Array(160)`，并额外记录 height=8、width=20。这里没有 RGBA 四个通道，每项直接表示笔画强度：0 是背景，1 是最强笔画，中间值表示灰度过渡。

显示时页面把强度 1 画成深色、0 画成浅色，方便像看黑字白纸一样阅读。这只是显示转换，模型仍接收原来的 0–1 强度。
...
公式中 X 是输入图像批次，K 是卷积核，A 是激活后的特征。下标 b 选第几张图，y/x 选空间位置，c 选输出通道；u/v 遍历 3×3 核。b_c 则是通道 c 的偏置，与样本下标 b 不是同一对象。输入通道只有一个，所以相关下标固定为 0。

在某个输出位置，先累计该核盖住的九个输入与权重的乘积，加一次偏置，再用 tanh。越界位置补 0。核不做数学卷积意义的翻转；与第15章和 `tf.conv2d` 约定一致。这里使用 tanh 便于观察正负特征，**不是原论文CNN的逐层复刻**。
...
先只算宽度或高度中的一条轴。N_in 是该轴输入有多少格，k 是核在该轴的长度，s 是移动步幅，两侧补格数分别为 P_left、P_right；d 是核元素间的间隔，普通连续核 d=1。下面的 floor 表示向下取整：只有完整窗口才贡献一个输出。

常见卷积/池化输出尺寸公式（有效核大小含膨胀率 d）：
...
这里 P 是池化结果，S 是重排后的序列；b 选样本，t 选列，y、c 选这一列中的行与通道。索引 `yC+c` 把每一行的 C 个通道接成一串。因此 `T=W'`、`F=H'×C`。H′=4、C=4 时，某一帧的 16 个特征排列为：第0行四通道、第1行四通道、第2行四通道、第3行四通道。
...
`transpose([0,2,1,3])` 中的数字是旧轴编号：0 保留批次，2 把宽度移到前面，1 把高度放到后面，3 保留通道。它真的重排了元素；随后的 reshape 才把已经相邻的“高度×通道”两条轴合在一起。

实现步骤：先读取池化实际 shape → 交换高宽轴 → 展平每列的高度与通道 → 校验某一帧等于原特征图相同列的连接。Node 实验5直接比较第一帧与池化列，出错会触发断言。
...
24×32 的来历是：每一步的 16 个列特征，加上旧状态的 8 项，一共 24 项输入；输入门 i、遗忘门 f、候选 g、输出门 o 各要给 8 个状态维度算数，共 `4×8=32` 个激活前分数。a_i 等符号就是各分支进入 Sigmoid/tanh 前的数，不是已经求好的门值。

这是第 18 章同一组公式的批张量版本。门顺序固定 i/f/g/o；遗忘门偏置初始化为1，其余门偏置为0。初始状态与cell全零。
...
式中的 y_b 是第 b 张图的整串答案，π 是逐帧路径，B(π) 是折叠路径的函数；这里 B(π) 与外层平均中的批次大小 B 含义不同。每条路径先把选中帧概率相乘，同一目标的合法路径再相加，最后取负对数；一批样本的损失再取平均。

第 19 章已经实现对数域前向表 α、后向表 β，以及 `∂L/∂z=p−γ`。p 是原始预测分布，γ 是已知目标条件下合法对齐的后验分布。本章调用同一个 `differentiableCtcLoss`，所有样本T=10，标签长度可以1、2或3，目标无需补齐成10帧。
...
这里的 grads 保存每个参数的梯度；norm 把所有梯度元素的平方加起来再开方，即全局 L2 范数。factor 是所有梯度共用的缩放倍数，最大为 1。

可以拿 `[6,8]` 作小例子：范数是 10，阈值是 5，factor≈0.5，于是裁剪后为 `[3,4]`。两个方向的比例保持不变；如果分别把每项截到 5，就会得到 `[5,5]`，已经改变方向。裁剪用于限制过大的一步，不能恢复消失的梯度，也不是重新定义损失。
...
`model/weights.json` 保存本章训练得到的 8 组数值：卷积核/偏置、正向 LSTM 矩阵/偏置、反向 LSTM 矩阵/偏置、输出矩阵/偏置。format 标记文件格式，config 记录模型尺寸，alphabet 记录每个输出索引代表哪个字符，每组 shape/values 则记录形状与具体参数值。无外部成品模型、无第17章依赖。`importWeights` 在赋值前校验配置、固定字符表 `['','0','1']`，以及全部张量的形状、数量、空槽和转成float32后的有限性，避免只加载了一半模型。JSON数值 `1e100` 在JavaScript中有限，却会变成float32的Infinity，因此只做 `Number.isFinite` 不够；调换字符表也必须拒绝，不能让相同输出索引代表不同字符。
...
访问 `http://127.0.0.1:4173/`，进入本章并打开[本章交互实验](index.html)。已有服务时直接访问。页面通过 HTTP 加载保存权重；先在默认图上识别并追踪一帧，再改像素观察输出。重新训练是独立操作，初次阅读不需要执行带 `--train` 或 `--write` 的命令。
