...
第25章将照片处理成原始数字串；第26章在预测完成后录入答案并调用本章评估。评估模块不调用识别器，也不把答案传回识别流程。阅读时先沿着 `1203 → 123` 算完一次评分，再进入词典和拒识实验；这三件事解决的问题不同，可以分别核对。

## 2. 先运行，再推导

首次使用仓库时，在项目根目录执行 `npm install` 安装工具依赖。随后从项目根目录运行：
...
在 `http://127.0.0.1:4173/` 打开第24章，再进入[本章交互实验](index.html)。已有开发服务时直接访问即可；启动脚本会准备实验资源。如果页面停留在“正在载入实验”，检查启动过程是否成功，并通过课程站访问，不要直接双击HTML文件。
...
先把两串分别命名：a是真值，长度为m；b是预测，长度为n。`a_i` 和 `b_j` 表示其中第i个、第j个字符，数学编号从1开始。在 `1203 → 123` 中，m=4、n=3。

计算整串差异前，先算较短片段的差异，并把答案存进表格，这就是这里的**动态规划**。“前缀”指从开头取若干项：真值的前3项是 `120`，预测的前2项是 `12`。因此表格里的 `d[3,2]` 问的是“把120变成12至少要改几次”，不是比较图像坐标，也不是只比较第三个和第二个字符。

默认比较单位是Unicode码点，可先理解为字符串中的文字编码单位；数字0–9各占一个码点。WER实验改用词，填表方法不变。
...
填表时先把第一行、第一列写成0、1、2……：这相当于从空串开始逐个添加，或把已有字符逐个删掉。然后从左上向右下计算内部格子。以下三个格子串起“先匹配12，再删0，最后匹配3”的过程：
...
表格的每个格子只比较三个候选，所以耗时主要取决于格子数。两串长度都翻倍，格子数大约变为4倍。这也解释了为什么界面限制输入长度：不仅要算出结果，还要让人读得清整张表。

下面的字符边界在数字串中不会造成歧义，但做多语言评估时必须知道：屏幕上看起来像“一个字”的东西，可能由多个码点组成。例如 `é` 既可能是一个码点，也可能是 `e` 加一个组合重音。归一化是把某些等价编码整理为统一形式；本模块不会自动做这件事。
...
先把每张照片的替换、删除、插入次数加起来，再除以所有照片实际写了多少个字符，这就是这里的microCER。`CER` 是字符错误率，`micro` 表示先合并计数再算比率。

公式中E是纳入评估的照片集合，k是照片编号，`N_k` 是第k张真值的字符数，必须大于0；`S_k、D_k、I_k` 分别是它的替换、删除、插入次数。求和符号表示把E中的每张照片都算进去：
...
两张图一共写了6个真实字符，其中漏了1个，因此聚合为 `1/(2+4)=1/6≈16.67%`。逐图比例算术平均是 `(0+1/4)/2=1/8=12.5%`，它让每张图权重相同，是另一种指标；不能用它冒充这里约定的 microCER。microCER也等于以真值长度为权重的单图CER平均。
...
这里的 `|E|` 表示照片张数，`1[条件]` 在整串完全相同时记1，否则记0。上例两张图只有A整串正确，所以为 `1/2=50%`。`00110` 只有原封不动的 `00110` 才正确；漏一个0或把连续1合成一个1都算整串错。
...
一张照片先要回答“能不能参加评分”，然后才回答“识别对不对”。这两个问题不能混在一起。例如，没有人工答案时无法判断对错；有答案但模型什么也没读出来时，已经可以确定发生了漏字。

每条记录只进入以下三种去向之一。`disposition` 就是“这条记录去哪一组”，判断顺序从表的第一行向下：
...
例如总共3张图：1张有答案但没有输出、1张未标注、1张损坏且写明排除理由。计数是“纳入1 + 待标注1 + 排除1 = 总共3”；那张没有输出的图已在“纳入1”中，不能另加一次。
...
第26章只需要把已经保存的真值和预测交给评估器，就能复用上面的规则。先看同一组两张图片怎样变成报告：
...
console.log(report.totalErrors);          // 1：总共漏了一个0
console.log(report.totalTruthCharacters); // 6：答案中共有6个字符
console.log(report.microCER);             // 1/6，约0.166667
console.log(report.exactMatchAccuracy);   // 0.5：两张中一张全对
```

第一次阅读先理解这四个值；需要接入页面或导出报告时，再展开完整字段。示例中的相对路径适用于章节目录内的脚本。

<details>
<summary>完整API参考：对齐结果、输入约束与返回字段</summary>
...
“原始”必须说清相对于哪一步。模型读出 `308` 后被策略整串拒识，字段会这样变化：

| 所在阶段 | 字段 | 保存什么 |
|---|---|---|
| 拒识策略 | `originalPrediction` | `308`，拒识前模型给出的候选 |
| 拒识策略交给评估器 | `prediction` | 空串，因为最终没有交付文本 |
| 评估器返回 | `rawPrediction` | 空串，表示评估器本次收到的原始输入 |

因此 `rawPrediction` 不保证是最初的模型候选。比较原始模型与拒识策略时，应分别送入两套prediction评分，不能混为一份成绩。
...
继续用 `the bock`：o是已经观察到的词 `bock`，w是候选原词 `book` 或 `back`，h是左侧上下文 `the`。我们要猜“哪一个原词更可能经过识别错误变成bock”。这叫noisy channel（噪声通道）：原文像信号，识别错误像传递过程加入的噪声。

需要同时考虑两件事：这个原词在上下文中有多常见，以及它变成当前观测词有多容易。贝叶斯公式把两者联系起来；`arg max` 表示选出让后面分数最大的候选，帽号w表示选中的结果：
...
`P(w|h)` 是只看上下文时原词的概率，称为先验；`P(o|w,h)` 是假设原词确实是w时，产生当前观测o的概率。比较不同w时，贝叶斯公式的共同分母不影响排名，所以可以省略。这两个因子对应语言模型与噪声机制。基于该思想的经典拼写纠错见 [Kernighan、Church、Gale 1990 原论文](https://aclanthology.org/C90-2036/)。OCR场景还应利用视觉证据或从开发数据估计的混淆概率；键盘拼错的统计不能直接当作手写识别错误统计。
...
预测 `read the book` 的最后一个词时，可以看整个历史 `read the`，也可以只看最近的 `the`。N-gram选择后一类简化：限制一次查看多少个相邻词。这里的gram指相邻项组合，不是图像特征维度。

把句子中的词依次记为 `w₁,…,wT`，T是词数，t是当前位置。整句概率可拆成“第一个词的概率 × 已知第一个词后第二个词的概率 × ……”，乘积符号Π就是把这些条件概率相乘：
...
先不用复杂模型，只数训练语料中“某个词后面跟了什么”。这里C表示次数，h表示上一词，w表示待预测的下一词。`C(h,w)` 是这对相邻词出现的次数，`C(h)` 是以h为上一词的总次数。

例如the后面总共出现过3次词，其中2次是book，那么按频率估计 `P(book|the)=2/3`。这种直接用观察频率估计概率的方法叫最大似然估计，简写MLE：

\[
P_{MLE}(w\mid h)=\frac{C(h,w)}{C(h)}
\]

问题是：没见过不代表不可能。如果某个组合的计数为0，直接估计会给它概率0，整句连乘也会归零。平滑会给每个可能的后继词先加一点“虚拟次数”。

把这点次数记为α，要求α>0；这里α是平滑参数，与图片透明度无关。V是可能的后继事件总数，每种都加α，就总共增加αV次：

\[
P_{add\text{-}\alpha}(w\mid h)=\frac{C(h,w)+\alpha}{C(h)+\alpha V}
\]

分子给某一种后继加α，分母给全部V种后继合计加αV，这样所有概率之和仍为1。默认 `α=1` 为add-one；它易于手算，但对大词表往往过度平滑，不能当作现代语言模型的完整平滑方案。
...
前两句提供2次 `the → book`，第三句提供1次 `the → back`，第四句没有the。因此 `C(the,book)=2`、`C(the,back)=1`、`C(the)=3`。默认α=1时，分母由3变成 `3+1×7=10`，于是：
...
book的平滑计数为3，back为2，其他5种后继各为1，总和 `3+2+5×1=10`，刚好与分母一致。

现在把概率放回第7节的排序公式。`bock`到两个候选距离都为1，编辑惩罚都是−1；设β=λ=1，book的得分是 `−1+ln(0.3)`，back是 `−1+ln(0.2)`：
...
两个分数都是负数，但−2.204比−2.609大，所以book排第一；这不是“负的概率”，而是距离惩罚与对数概率相加的排序分数。设λ=0时两者都为−1，规则按距离再按JS字符串字典序稳定排序，back排第一。这是实验结果，并非宣称bock在现实文本中必然拼错。
...
这次策略不改字，只决定“整串自动交付，还是交给人工复核”。逐字符查看模型得分，只要有一个低于阈值，就整串拒识。这样能指出不确定的结果，但不能自动知道正确答案。

设一串共有n个字符，得分依次为 `p₁,…,pₙ`。`c_min` 表示其中最低的得分，τ表示允许自动交付的最低门槛，accept为1表示接受、为0表示拒识：
...
按原始输出，B只把5读成8，发生1次替换，因此CER为 `1/(2+3)=20%`。拒识后B没有交付任何文本，相对于真值305就是3次删除，CER变为 `3/5=60%`。两种情况下都只有A整串正确，所以整串正确率仍为1/2。

如果只展示自动交付的A，会看到1/1=100%。这还必须同时说清：系统只自动交付了全部照片的一半。**覆盖率**就是自动接受的样本占比，本例为1/2：
