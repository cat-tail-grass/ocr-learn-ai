...
async function copyCommand() {
  const command = course.value?.runCommand;
  if (!command) return;
  ...
  try {
    ...
    if (course.value?.runCommand !== command) return;
    copyLabel.value = '已复制';
  ...
  <ThemeLayout :class="{ 'course-layout': course, 'course-home': isCatalogPage }">
    ...
    <template #page-content-top>
      <header v-if="course" id="course-chapter-start" class="course-chapter-header" aria-label="本章学习入口">
        ...
        <div v-if="course.labPath" class="course-chapter-actions">
          <a
        ...
        <div v-if="course.runCommand" class="course-node-command">
          <span class="course-command-label">本地运行</span>
          <code tabindex="0">{{ course.runCommand }}</code>
          <button type="button" @click="copyCommand">{{ copyLabel }}</button>
        </div>
        <p v-if="course.runCommand" class="course-command-help">在项目根目录执行。<span role="status" aria-live="polite">{{ copyMessage }}</span></p>
      </header>
